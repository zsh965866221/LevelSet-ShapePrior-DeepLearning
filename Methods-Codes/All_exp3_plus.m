...
for index=1:length(testlist)
    ...
    if exist(['./Output_PortraitFCNplus/' sprintf('%05d',imgindex) '_output.mat'],'file')
		...
		load(['./Output_PortraitFCNplus/' sprintf('%05d',imgindex) '_output.mat']); % res
		load(['./Affined_shapemaskplus/' sprintf('%05d',imgindex) '_affinedshape.mat']); % affined_mask
		img = rgb2gray(img);
		img = double(img);
		%%
		...
		%%
		for n = 1:iterNum
			u = exp3_RSF_plus(u, I, K, KI, KONE, P1, P2, timestep, epsilon, lambda1, lambda2, rho1, rho2, pi1, pi2, affined_mask, 1-affined_mask, nu, mu, 1);
		end
		%%
...
save('./IoU_PortraitFCNplus/IoU.mat', 'All_IoU', 'All_IoU_Portrait');