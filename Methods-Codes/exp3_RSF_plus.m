function u = exp3_RSF_plus(u0, Img, Ksigma, KI, KONE, P1, P2, timestep, epsilon, lambda1, lambda2, rho1, rho2, pi1, pi2, mask1, mask2, nu, mu, numIter)
u = u0;
for j = 1:numIter
...
%     dataForce = (lambda1 - lambda2) * KONE .* Img .* Img + conv2(s1, Ksigma, 'same') - 2.* Img .* conv2(s2, Ksigma, 'same');
    ...
    D = -DrcU .* (pi1 .* mask1.*P1 - pi2 .* mask2.*P2);
